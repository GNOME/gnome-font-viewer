                                   ...
                                   GError *error)
{
    ...
    if (error != NULL) {
        ...
        if (font_view_model_get_iter_for_face (FONT_VIEW_MODEL (self->model), face, NULL)) {
            ...
            gtk_button_set_label (GTK_BUTTON (self->install_button), _("Installing…"));
            gtk_widget_set_sensitive (self->install_button, FALSE);
        } else {
                               ...
                               gint face_index)
{
    ...
    if (self->install_button == NULL) {
        ...
        gtk_widget_set_size_request (self->install_button, 100, -1);
...
static void
font_view_application_finalize (GObject *obj)
{
    FontViewApplication *self = FONT_VIEW_APPLICATION (obj);

    g_clear_object (&self->cancellable);

    G_OBJECT_CLASS (font_view_application_parent_class)->finalize (obj);
}
...
font_view_application_class_init (FontViewApplicationClass *klass)
{
    ...
    oclass->finalize = font_view_application_finalize;
}
